%%% Function of Detect watermarks with FE to formant and QIM to residue
%%% By WANG Shengbei
%%% 2015 7/24

function  [Real_WM_LSFs Real_WM_Res Real_WM_LSFs_Res]=Detect_Function_QIM_FE(WM_speech, p)

Detected_WMs_LSFs=[];
Detected_WMs_Res=[];

Yt_Length = length(WM_speech);
FrameLeng=p.FrameLeng;
Frame_Num=floor(Yt_Length/FrameLeng);


for k=1:Frame_Num
    WM_speech_frame=WM_speech((k-1)*FrameLeng+1:k*FrameLeng);
    [WM_LP_coef,g]=lpc(WM_speech_frame,p.LPorder);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
%%%%%%%%%%%%Detecting watermarks from formants (FE)%%%%%%%%%%%%%%%%%%%%%%%%%

    WM_LSPs=poly2lsf_my(WM_LP_coef); %lpc to lsp in Pi
    WM_LSPs_m=WM_LSPs(3:8);

    WM_ds=zeros(5,1);
       for j=1:5 
           WM_ds(j)=abs(WM_LSPs_m(j+1)-WM_LSPs_m(j));
       end
    WM_Ds=WM_ds; 
  
%%%%%%%%find st_num minimum distances  %%%%%%%%%
    posit=zeros(p.Num,1);
       for jj=1:p.Num
           t=find((WM_ds-min(WM_ds))==0);
           posit(jj)=t(1);% in case of there are same distance, then t is not a single value
           WM_ds(t(1))=3.14;
       end  
  
%%%%%%%%% calculation of watermarks %%%%%%%%%%%%%%%%%%%%%%%
    pp_1=posit(1);
    pp_2=posit(2);
  
    D=abs(WM_Ds(pp_1)-WM_Ds(pp_2));
   if D<WM_Ds(pp_1)*(p.FEstep/2-1)/(p.FEstep/2)
      Detected_WMs_LSFs=[Detected_WMs_LSFs;1]; 
   else 
      Detected_WMs_LSFs=[Detected_WMs_LSFs;0];
   end   
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
%%%%%%%%%%%% Detection watermarks from Residue (QIM)%%%%%%%%%%%%%%%%%%%%%%%
    WM_Residue=filter([1 WM_LP_coef(2:end)],1,WM_speech_frame);
    Residue_WM_fft = fft(WM_Residue);
 
    WM_Residue_Mag = (2/FrameLeng)*abs(Residue_WM_fft);
    WM_Residue_Phs = angle(Residue_WM_fft);
    Embedded_bins = (2:p.Freqbin+1)';%pos_bins = (2:p.Freqbin+1)'; % positive frequency bins for embedding
    [WM_Res]= QIMDecode(WM_Residue_Mag(Embedded_bins),WM_Residue_Phs(Embedded_bins),p.QIMsteps);
    Detected_WMs_Res=[Detected_WMs_Res;WM_Res]; 

end

% get the correct watermark length
WMs_LSFs_length=floor(length(Detected_WMs_LSFs)/p.Dup);
Detected_WMs_LSFs=Detected_WMs_LSFs(1:WMs_LSFs_length*p.Dup);

WMs_Res_length=floor(length(Detected_WMs_Res)/p.Dup);
Detected_WMs_Res=Detected_WMs_Res(1:WMs_Res_length*p.Dup);

Reshp_WMs_LSFs=reshape(Detected_WMs_LSFs',p.Dup,WMs_LSFs_length);
Reshp_WMs_Res=reshape(Detected_WMs_Res',p.Dup,WMs_Res_length);

% Watermarks from LSFs
Mean_WMs_LSFs=mean(Reshp_WMs_LSFs);
for kkk=1:length(Reshp_WMs_LSFs)
    if Mean_WMs_LSFs(kkk)>=0.5
       Real_WM_LSFs(kkk)=1;
    else
       Real_WM_LSFs(kkk)=0;
    end
end

% Watermarks from Res
Mean_WMs_Res=mean(Reshp_WMs_Res);
for kkk=1:length(Reshp_WMs_Res)
    if Mean_WMs_Res(kkk)>=0.5
       Real_WM_Res(kkk)=1;
    else
       Real_WM_Res(kkk)=0;
    end
end

% Watermarks from LSFs and Res
Mean_WMs_LSFs_Res=mean([Reshp_WMs_LSFs;Reshp_WMs_Res]);
for kkk=1:length(Reshp_WMs_LSFs)
    if Mean_WMs_LSFs_Res(kkk)>=0.5
       Real_WM_LSFs_Res(kkk)=1;
    else
       Real_WM_LSFs_Res(kkk)=0;
    end
end