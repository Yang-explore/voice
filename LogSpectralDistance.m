...
%LSD=mean(sqrt(mean((20*log10(CL(RangeBin,1:N))-20*log10(NO(RangeBin,1:N))).^2)));


LSDtmp=mean((log(CL(RangeBin,1:N))-log(NO(RangeBin,1:N))).^2);
LSD=mean(sqrt(LSDtmp( (isnan(LSDtmp)~=1)&(isinf(LSDtmp)~=1) )));
