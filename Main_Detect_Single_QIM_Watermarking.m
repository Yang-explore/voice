%%% Main entrance of Single Watermarking embedding with QIM(Ngo)
%%% By WANG Shengbei
%%% 2015 7/24
%%% Shengbei Wang
%%% Update  21st, Nov. 2016 
...
WM_InFolder  = 'E:\ALL_Program_WANG\Hybrid_Watermarking_QIM_FE\Proposed_QIM_FE\Single_WM_QIM_8.1_sec_speech\';
...
for k = 1:12 
     for kk=1:Bit_list_length
           Bps=Bit_List(kk);
           Real_Bps=Bps/4;
           WM_speech=[WM_InFolder 'WM_QIM_Bps' num2str(Real_Bps) '_00' num2str(k) '.wav'];
           ...
           [Real_WM_Res]=Detect_Function_Single_QIM(yt, p);
           ...
           Detect_WM_Leng = length(Real_WM_Res);
           ...
           Detect_error=xor(Real_WM_Res',WMs_Embedded);
           Error_sum=sum(Detect_error);

           clear correct_rate;
           Correct_rate_QIM=100 * (Detect_WM_Leng- Error_sum) / Detect_WM_Leng; 
...
cd(Result_Folder);
save Single_QIM_BDR Bit_detection_rate_QIM
result_text=['Single_QIM_BDR.txt'];
